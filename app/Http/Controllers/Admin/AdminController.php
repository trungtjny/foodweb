...
class AdminController extends Controller
{
    ...
    public function shop($id) {
        $arrMoney =DB::select('SELECT SUM(totalprice) as totalmoney, DATE(created_at) as day FROM orders WHERE status = ? AND shop_id = ? AND EXTRACT(MONTH FROM created_at) = ? group by day', [0, $id, Date('m')]);
    }
}
